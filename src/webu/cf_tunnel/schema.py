...
CF_TUNNEL_CONFIG = ConfigSpec(
    ...
    notes=[
        ...
        "低延迟稳定性当前推荐基线：origin_request.connect_timeout=5、keep_alive_connections=256、keep_alive_timeout=120，并配合 cloudflared_run.protocol=http2、edge_ip_version=4、dns_resolver_addrs=[1.1.1.1:53, 1.0.0.1:53] 作为首选起点。",
        "这组参数是当前验证过的首选起点，不代表所有网络都能永久保持 <1s；若访客侧仍抖动，继续用 edge-trace、snapshot 和 client-canary 做分群测量与回滚。",
