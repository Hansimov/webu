...
def render_usage_markdown() -> str:
    lines = [
        ...
        "- 每个 tunnel 都应安装独立的 `cloudflared-tunnel-*.service`，并在 `cftn tunnel-apply --install-service --save-config` 之后立即用 `cftn tunnel-status --name app-example` 确认控制面返回 `healthy` 且存在活动连接。",
        "- 对公开业务链路，优先监控 `local_url` 可用性、`tunnel-status` 和 `snapshot` 结果，不要用同机公网回环探测替代源站健康检查。公网回环更容易混入本机 DNS、出口和浏览器缓存噪声。",
        "- 如果延迟升高或开始出现超时，先运行 `cftn snapshot --name app-example --prefer-family any` 固化现场，再看是否需要用 `edge-trace`、`client-override-plan` 和 `client-canary-bundle` 进入访客侧分群优选。",
        "",
        ...
        "",
        "1. 先固化源站侧基线：`cftn tunnel-apply --name app-example --install-service --save-config`，确保配置里已经写入推荐的 `origin_request` 和 `cloudflared_run` 参数。",
        "2. 用 `cftn tunnel-status --name app-example` 确认 tunnel 为 `healthy`，并且重启后能快速恢复活动连接。如果这里已经不稳，不要先做访客侧优选。",
        "3. 用 `cftn snapshot --name app-example --prefer-family any` 固化当前 `edge-trace`、家族判断、候选 IP 和免费外部检测入口，给后续排障留证据。",
        "4. 只有在源站侧稳定但访客侧仍然慢或抖动时，再用 `cftn client-override-plan` 或 `cftn client-canary-bundle` 做 `地区 + 运营商 + 平台` 分层灰度。",
        "5. 任何候选只要出现证书错误、超时升高、成功率下降或 `trace_colo` 漂移明显，就立即回滚客户端 override，并保留一批正常 DNS 对照用户做持续复测。",
        "6. 把 `snapshot`、客户端回传和 `client-report-summary` 当成周期性动作，而不是一次性调参。Anycast 漂移是常态，优选结果需要持续刷新。",
        "",
        "## What Changes",
        "",
