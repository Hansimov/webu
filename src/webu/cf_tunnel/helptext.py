...
COMMAND_HELP = {
    ...
    "tunnel-stabilize": {
        ...
        "examples": [
            ...
            "cftn tunnel-stabilize --name app-example --no-install-service",
        ],
    },
    "tunnel-guard": {
        "summary": "常驻后台巡检 tunnel 状态，周期性留快照，并在连续异常时自动触发 stabilize。",
        "examples": [
            "cftn tunnel-guard --name app-example --interval-seconds 60",
            "cftn tunnel-guard --name app-example --snapshot-interval-seconds 1800",
        ],
    },
    "access-diagnose": {
    ...
    "client-override-plan": {
        "summary": "导出面向特定用户群的客户端 hosts/本地 DNS 优选 IP 实验计划，并输出 recommended_prefer_family 提示。",
        "examples": [
    ...
    "client-canary-bundle": {
        "summary": "生成跨桌面和移动端的 canary 测试包、平台指引、recommended_prefer_family 和灰度步骤。",
        "examples": [
            "cftn client-canary-bundle --name app-example --prefer-family ipv4",
            "cftn client-canary-bundle --hostname app.example.com --prefer-family any --max-candidates 3",
        ],
    },
    ...
    return assert_public_text_safe(
        root_epilog(
            ...
            examples=[
                ...
                "cftn access-diagnose --name app-example",
                "cftn page-audit --name app-example",
                "cftn client-override-plan --name app-example --prefer-family any",
                "cftn client-canary-bundle --name app-example --prefer-family any",
                "cftn snapshot --name app-example --prefer-family any",
            ],
        )
    )
...
def render_usage_markdown() -> str:
    lines = [
        ...
        "6. 如果希望 webu 自己 24x7 巡检并在连续异常时自动回正，运行：`cftn tunnel-guard --name app-example --interval-seconds 60`。使用 `tunnel-apply --install-service` 时，默认也会安装 companion `cloudflared-tunnel-guard-*.service`，除非显式加 `--no-guard-service`。",
        "7. 如果要手动做一次根因判定、按需重放基线并补快照，运行：`cftn tunnel-stabilize --name app-example`。",
        "8. 如果大陆访问慢、必须挂代理或浏览器提示连接不安全，运行：`cftn access-diagnose --name app-example`。",
        "9. 如果页面本身异常、空白、资源报错或疑似 mixed content，运行：`cftn page-audit --name app-example`。",
        "10. 如果要继续判断当前网络命中的 Cloudflare 边缘、colo 和是否适合做客户端侧优选实验，运行：`cftn edge-trace --name app-example`。",
        "11. 如果要先让工具判断当前网络更适合 IPv4 还是 IPv6，再把结果转成定向用户可执行的实验包，运行：`cftn client-override-plan --name app-example --prefer-family any`。",
        "12. 如果要覆盖桌面和移动端灰度，并带上 recommended_prefer_family 与回传模板，生成 canary 测试包：`cftn client-canary-bundle --name app-example --prefer-family any`。",
        "13. 如果要把当前测量结果、首轮灰度候选和免费外部检测入口固化成一份操作快照，运行：`cftn snapshot --name app-example --prefer-family any`。",
        "14. 收集客户端结果后，汇总各地区/运营商/平台表现：`cftn client-report-summary reports/client-canary.json`。",
        "15. 用 `dig example.com NS`、`dig +trace example.com A` 和实际业务 URL 做最终验证。",
        "",
        "## Common Commands",
        "",
        ...
        "- 做一次判定 + 按需回正：`cftn tunnel-stabilize --name app-example`",
        "- 常驻后台巡检并自动修复：`cftn tunnel-guard --name app-example --interval-seconds 60`",
        ...
        "- 如果不确定该先测 IPv4 还是 IPv6，先用 `--prefer-family any`。`cftn client-override-plan` 和 `cftn client-canary-bundle` 会输出 `recommended_prefer_family` 与 `family_assessment`，比凭记忆硬编码家族更可靠。",
        "- `cftn client-override-plan` 会把当前成功的 Cloudflare 边缘候选导出成 hosts 覆盖和验证命令，适合给小规模大陆用户灰度试验。",
        "- `cftn client-canary-bundle` 会把桌面端 hosts 覆盖、移动端本地 DNS 灰度方式、验证步骤、recommended_prefer_family 和回传模板打包成一份结果。",
        "- 文档、测试和历史报告里的 IP 字面量只用于样例说明，不能直接当成生产优选候选。真实候选必须来自当前网络重新执行 `edge-trace` / `client-override-plan` 的结果。",
        ...
        "",
        "- 没有任何单一 cloudflared 参数组合可以对所有地区、运营商和时段永久保证 `<1s` 与 `0` 超时。对 Tunnel 更现实的目标是先把 `源站 -> Cloudflare edge` 这段尽量稳定，再对访客侧做持续测量和小流量回滚。",
        "- 当前已验证的源站侧首选基线是：`origin_request.connect_timeout=5`、`origin_request.keep_alive_connections=256`、`origin_request.keep_alive_timeout=120`。这组参数的目标是更快暴露坏连接、减少短连接抖动，并把健康连接尽量复用起来。",
        "- 当前已验证的 cloudflared 运行首选基线是：`cloudflared_run.protocol=http2`、`cloudflared_run.edge_ip_version=4`、`cloudflared_run.dns_resolver_addrs=[1.1.1.1:53, 1.0.0.1:53]`。这组设置更适合作为大陆网络优先的起点。",
        "- 每个 tunnel 都应安装独立的 `cloudflared-tunnel-*.service`，并在 `cftn tunnel-apply --install-service --save-config` 之后立即用 `cftn tunnel-status --name app-example` 确认控制面返回 `healthy` 且存在活动连接。默认情况下，这个安装流程还会附带一个 `cloudflared-tunnel-guard-*.service` sidecar，让 webu 自己周期巡检并在连续异常时自动触发 stabilize。",
        "- 对公开业务链路，优先监控 `local_url` 可用性、`tunnel-status` 和 `snapshot` 结果，不要用同机公网回环探测替代源站健康检查。公网回环更容易混入本机 DNS、出口和浏览器缓存噪声。",
        "- 如果延迟升高或开始出现超时，优先保证 `tunnel-guard` 正在运行；它会先盯住 tunnel 控制面和本地 HTTPS 路径，在连续异常时自动重放已验证基线，并补一份 fresh snapshot。",
        "- 如果问题仍然集中在访客侧网络差异，而不是 tunnel 控制面或源站，可继续用 `edge-trace`、`client-override-plan` 和 `client-canary-bundle` 进入访客侧分群优选。",
        "",
        ...
        "- 如果两个 hostname 当前都得到同一个 `recommended_prefer_family`，但原因不同，也先不要急着拆成不同默认策略。先保持同一个首轮家族，把差异留给客户端回传和 `client-report-summary` 去验证。",
        ...
        "",
        "1. 先固化源站侧基线：`cftn tunnel-apply --name app-example --install-service --save-config`，确保配置里已经写入推荐的 `origin_request` 和 `cloudflared_run` 参数，并让 companion guard sidecar 一起装好。",
        "2. 用 `cftn tunnel-status --name app-example` 确认 tunnel 为 `healthy`，并且重启后能快速恢复活动连接。如果这里已经不稳，不要先做访客侧优选。",
        "3. 让 `cftn tunnel-guard --name app-example --interval-seconds 60` 常驻运行。它会定期抓状态、按周期留 snapshot，并在连续异常时自动触发 `tunnel-stabilize`。",
        "4. 如果要人工补一次根因判定或立即刷新一次回正流程，运行 `cftn tunnel-stabilize --name app-example`。",
        "5. 只有在源站侧稳定但访客侧仍然慢或抖动时，再用 `cftn client-override-plan` 或 `cftn client-canary-bundle` 做 `地区 + 运营商 + 平台` 分层灰度。",
        "6. 任何候选只要出现证书错误、超时升高、成功率下降或 `trace_colo` 漂移明显，就立即回滚客户端 override，并保留一批正常 DNS 对照用户做持续复测。",
        "7. 把 `snapshot`、客户端回传和 `client-report-summary` 当成周期性动作，而不是一次性调参。Anycast 漂移是常态，优选结果需要持续刷新。",
        "",
        "## What Changes",
        "",
        ...
        "3. 用 `cftn client-override-plan --name app-example --prefer-family any` 先拿到 `recommended_prefer_family`。如果输出推荐 `ipv4`，首轮就先发 IPv4 cohort；如果推荐 `ipv6`，也要和 IPv4 分开测。",
        "4. 用 `cftn client-canary-bundle --name app-example --prefer-family any` 生成首轮多端测试包。桌面端默认走 hosts 覆盖；Android 和 iOS 默认走测试 Wi-Fi 或路由器 DNS override，而不是要求普通手机直接改 hosts。若想让首轮更干净，可按推荐家族再生成单家族 bundle。",
        "5. 每个 cohort 一次只测一个候选 IP，不要在同一批用户里同时下发多个候选，否则回传结果会混淆。每组先做 3 到 10 人小灰度，确认成功率和 TTFB 再扩大。",
        "6. 让每个测试用户回传统一字段：`region`、`city`、`isp`、`platform`、`network_type`、`candidate_ip`、`success`、`ttfb_ms`、`trace_colo`、`trace_loc`、`cf_ray`。模板可由 `cftn client-report-template --name app-example --prefer-family any` 生成。",
