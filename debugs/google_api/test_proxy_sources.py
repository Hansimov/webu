...
async def test_proxy_with_endpoint(proxy_url: str, protocol: str, endpoint_url: str, expect_status: int, expect_body: str | None) -> dict:
    """测试单个代理+端点组合。"""
    result = {"proxy": proxy_url, "url": endpoint_url, "ok": False, "latency_ms": 0, "error": "", "body": ""}
...
async def main():
    ...
    for key, ips in sorted(groups.items()):
        ...
        for ip in sample:
            ...
            tasks.append(test_proxy_with_endpoint(proxy_url, protocol, endpoint_url, 200, "origin"))
        ...
        for r in results:
            if r["ok"]:
                logger.okay(f"    ✓ {r['proxy']} → {r['body'][:80]} ({r['latency_ms']}ms)")
            else:
    ...
    for key, ips in sorted(groups.items()):
        ...
        for ip in sample:
            proxy_url = _build_proxy_url(ip["ip"], ip["port"], ip["protocol"])
            tasks.append(test_proxy_with_endpoint(
                proxy_url, protocol,
                "http://www.google.com/generate_204", 204, None
            ))
