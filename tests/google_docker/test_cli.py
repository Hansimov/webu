...
def test_prepare_space_bundle_excludes_configs(tmp_path):
    ...
    (source_root / "src" / "webu" / "google_docker" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "captcha" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "captcha" / "imgs").mkdir(parents=True)
    (source_root / "src" / "webu" / "captcha" / "imgs" / "verify.jpg").write_text(
        "img\n", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "fastapis" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "runtime_settings" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "gemini" / "secret.py").write_text(
        "secret\n", encoding="utf-8"
    )
    (source_root / "src" / "webu" / "warp_api" / "server.py").write_text(
        "warp\n", encoding="utf-8"
    )
...
def test_prepare_space_bundle_includes_encrypted_profile_bootstrap(
    ...
    monkeypatch.setattr(
        ...
        lambda runtime_env=None, service_type=None: SimpleNamespace(
...
def test_prepare_space_bundle_falls_back_to_tracked_profile_asset(
    tmp_path, monkeypatch
):
    source_root = tmp_path / "repo"
    ...
    monkeypatch.setattr(
        "webu.google_docker.cli.resolve_google_api_settings",
        lambda runtime_env=None, service_type=None: SimpleNamespace(
            profile_dir=missing_profile_dir
        ),
    )
    monkeypatch.setattr(
        "webu.google_docker.cli.resolve_google_api_service_profile",
    ...
    monkeypatch.setattr(
        "webu.google_docker.cli.TRACKED_PROFILE_ARCHIVE_PATH", tracked_archive_path
    )
...
def test_prepare_local_docker_build_context_includes_shared_bootstrap(
    tmp_path, monkeypatch
):
    source_root = tmp_path / "repo"
    tracked_source_dir = tmp_path / "tracked-profile"
...
def test_cli_parser_supports_factory_restart_during_sync():
    ...
    args = parser.parse_args(
        ["hf-sync", "--space", "owner/demo", "--restart", "--factory"]
    )
...
def test_cli_parser_supports_hf_super_squash():
    parser = build_parser()
    args = parser.parse_args(
        ["hf-super-squash", "--space", "owner/demo", "--branch", "main"]
    )
    assert args.space == "owner/demo"
    ...
    monkeypatch.setattr(
        ...
        lambda space_name: (
            ...
            HfSpaceSettings(
                repo_id=space_name,
                hf_token="hf_xxx",
                space_host="https://example.hf.space",
            ),
        ),
    )
    monkeypatch.setattr(
...
def test_cmd_hf_sync_all_runs_in_parallel(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "webu.google_docker.cli._refresh_tracked_profile_asset",
        lambda preferred_spaces=None: {"action": "unchanged"},
    )
...
def test_cmd_hf_check_renders_combined_status(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        ...
        lambda space_name: (
            _Api(),
            SimpleNamespace(space_host="https://space-host.example", hf_token="hf_xxx"),
        ),
    )
    monkeypatch.setattr(
        "webu.google_docker.cli._resolve_default_space_name",
        lambda explicit=None: "owner/demo",
    )
    monkeypatch.setattr(
        "webu.google_docker.cli._resolve_hf_service_url",
        lambda space_name=None: "https://space-host.example",
    )
    monkeypatch.setattr(
        "webu.google_docker.cli._resolve_admin_token",
        lambda explicit=None: "admin-token",
    )
    monkeypatch.setattr("webu.google_docker.cli.requests.get", _fake_get)

    cmd_hf_check(
        SimpleNamespace(
            space="", admin_token="", timeout=30, query="OpenAI news", check_auth=True
        )
    )
    output = capsys.readouterr().out
...
def test_cmd_docker_check_reports_direct_process_hint(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "webu.google_docker.cli._detect_port_listener",
        lambda port: "LISTEN 0 128 127.0.0.1:18200 users:(('python',pid=1234,fd=8))",
    )
    monkeypatch.setattr(
...
def test_config_schema_validator_reports_errors():
    errors = validate_config_payload(
        "google_api", {"host": "0.0.0.0", "port": "18200", "services": []}
    )
