...
├── server.py        # 服务层：FastAPI REST 接口（22 个端点，837 行）
├── client.py        # 客户端层：同步 HTTP 封装（requests，402 行）
...
| `/download_images` | POST | 图片 | 下载最新响应图片保存到本地 |
| `/screenshot` | POST | 调试 | 截图 |
