...
3. 把它们拆出来后，`ggdk` 和 `gghb` 的边界更稳定：前者管部署，后者管 hub 行为。

## 运行模型

1. `ggdk hub-docker-up` 负责把 hub 以容器形式拉起。
2. `gghb check` / `gghb backends` / `gghb search` / `gghb benchmark` 负责对这个 hub 实例做真实检查。
3. 如果需要非容器前台调试，则直接使用 `gghb serve`。

## Benchmark 目标

`gghb benchmark` 关注两类结果：
