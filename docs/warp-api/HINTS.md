...
```bash
sudo resolvectl domain <primary_uplink> ~.
```

**原理**：systemd-resolved 中，接口级 routing domain 优先于全局 routing domain。
将 `~.` 设置在 `<primary_uplink>` 上后，该接口的 ISP DNS 成为所有域名查询的首选，
...
```bash
resolvectl status <primary_uplink>
# DNS Domain: ~.
...
ip -6 rule add priority 5200 from <primary_ipv6_prefix>::/64 lookup main
